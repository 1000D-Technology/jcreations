...
function DailyDeals() {
    ...
    useEffect(() => {
        const fetchDailyDeals = async (attempt = 0) => {
            try {
                setLoading(true);
                setError(null);
                
                // Check if data is cached (simple 5-minute cache)
                const cacheKey = 'dailyDeals';
                const cacheTimeKey = 'dailyDealsTime';
                ...
                // Fetch daily deals from the API
                const response = await api.get('/deals/8');
                ...
                console.error(`Error fetching daily deals (attempt ${attempt + 1}):`, err);
                ...
                if (isConnectionError && attempt < MAX_RETRIES) {
                    ...
                    await wait(delay);
                    return fetchDailyDeals(attempt + 1);
                } else {
                        ...
                        <div className="flex overflow-x-auto gap-4 mt-4 pb-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent lg:grid lg:grid-cols-4 lg:overflow-x-visible lg:gap-6">
                            {products.map((product) => (
                                <div key={product.id} className="flex-shrink-0 w-[280px] lg:w-full relative">
                                    {/* Daily Deal Badge */}
                                    <div className="absolute top-2 left-2 z-10 bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs px-2 py-1 rounded-full shadow-lg">
                                        Daily Deal
                                    </div>
